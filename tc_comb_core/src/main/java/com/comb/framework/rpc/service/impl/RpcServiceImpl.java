...
public class RpcServiceImpl extends RpcServiceGrpc.RpcServiceImplBase {
	...
	//private DisruptorService disruptorService = new DisruptorServiceImpl();
	
	@Override
    public void requestFunc(RpcProto.RequestMsg request, StreamObserver<RpcProto.ResponseMsg> responseObserver){
    ...
    @Override
    public void requestFuncWithConfirmCode(RpcProto.RequestMsg request, StreamObserver<RpcProto.ConfirmMsg> responseObserver){
        ByteString req = request.getRequest();
        //此函数返回确认消息，确认远程调用成功
    ...
    private void PushResult(StreamObserver<RpcProto.ResponseMsg> responseObserver, byte[] retMsg){
        ...
        responseMsg = RpcProto.ResponseMsg.newBuilder().setResponse(ByteString.copyFrom(retMsg)).build();
    ...
    private void PushResult(StreamObserver<RpcProto.ConfirmMsg> responseObserver, boolean result){
        ...
        responseMsg = RpcProto.ConfirmMsg.newBuilder().setResult(result).build();
        responseObserver.onNext(responseMsg);
        responseObserver.onCompleted();
    }
