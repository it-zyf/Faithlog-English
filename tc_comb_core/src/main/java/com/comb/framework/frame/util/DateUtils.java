...
public class DateUtils {
	...
	private static final List<String> FULL_MONTH = Arrays
			.asList(new String[] { "01", "02", "03", "04", "05", "06", "07", "08", "09", "10", "11", "12" });
	...
	public static Date parseDate(final String date) {
		try {
			return org.apache.commons.lang3.time.DateUtils.parseDate(date, new String[] { DEFAULT_DATE_FORMAT_PATTERN });
	...
	/**
	 * 数字不满位数自动补零. example autoAddPrefixZero(1, 4) result: 0001
	 * 
	 * @param num
	 * @param digit
	 * @return
	...
	public static Long getDifferDays(String beginDate, String endDate) {
		...
		try {
			diffday = (formatter.parse(endDate).getTime() - formatter.parse(beginDate).getTime())
					/ (1000 * 24 * 60 * 60);
		} catch (ParseException e) {
			// e.printStackTrace();
			log.error("日期解析失败.", e);
	...
	public static String getStringDateShort2(Date date) {
		SimpleDateFormat formatter = new SimpleDateFormat("yyyy/MM/dd");
		String dateString = formatter.format(date);
		return dateString;
	}

	public static String getStringDateShort3(Date date) {
	...
	public static String strToDateToStr(String strDate, String splitChar) {
		SimpleDateFormat formatter = new SimpleDateFormat("yyyyMMdd");
		ParsePosition pos = new ParsePosition(0);
		Date strtodate = formatter.parse(strDate, pos);
		formatter = new SimpleDateFormat("yyyy" + splitChar + "MM" + splitChar + "dd");
		strDate = formatter.format(strtodate);
		return strDate;
	}
