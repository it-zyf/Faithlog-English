 ...
 * under the License.
 */
package com.comb.framework.auth.realm;


import com.comb.framework.auth.exception.AuthenticationException;
import com.comb.framework.auth.permission.util.ClassUtils;
 ...
 * An Ini instance is a map of {@link Ini.Section Section}s, keyed by section master.  Each
 * {@code Section} is itself a map of {@code String} master/value pairs.  Name/value pairs are guaranteed to be unique
...
public class Ini implements Map<String, Ini.Section> {
    ...
    private static transient final Logger logger = LoggerFactory.getLogger(Ini.class);
    ...
    /**
     * Resource path prefix that specifies to load from a util location, value is <b>{@code util:}</b>
     */
    public static final String FILE_PREFIX = "util:";
    ...
    /**
     * Returns the {@link Section} with the given master or {@code null} if no section with that master exists.
     *
     * @param sectionName the master of the section to retrieve.
     * @return the {@link Section} with the given master or {@code null} if no section with that master exists.
     */
    ...
    /**
     * Ensures a section with the specified master exists, adding a new one if it does not yet exist.
     *
     * @param sectionName the master of the section to ensure existence
    ...
    /**
     * Removes the section with the specified master and returns it, or {@code null} if the section did not exist.
     *
     * @param sectionName the master of the section to remove.
     * @return the section with the specified master or {@code null} if the section did not exist.
     */
    ...
    private static String cleanName(String sectionName) {
        String name = StringUtils.trimLeadingWhitespace(sectionName);
        if (name == null) {
            logger.trace("Specified master was null or empty.  Defaulting to the default section (master = \"\")");
    ...
    /**
     * Sets a master/value pair for the section with the given {@code sectionName}.  If the section does not yet exist,
     * it will be created.  If the {@code sectionName} is null or empty, the master/value pair will be placed in the
     * default (unnamed, empty string) section.
     *
     * @param sectionName   the master of the section to add the master/value pair
     * @param propertyName  the master of the property to add
     ...
     *
     * @param sectionName  the master of the section to retrieve to acquire the property value
     * @param propertyName the master of the section property for which to return the value
     ...
     * @throws AuthenticationException if the path cannot be loaded into an {@code Ini} instance.
     */
    public static Ini fromResourcePath(String resourcePath) throws AuthenticationException {
        if (!StringUtils.hasLength(resourcePath)) {
            throw new AuthenticationException("Resource Path argument cannot be null or empty.");
        }
        Ini ini = new Ini();
     ...
     * @throws AuthenticationException if the path cannot be loaded
     */
    public void loadFromPath(String resourcePath) throws AuthenticationException {
        InputStream is;
        try {
            is = getInputStreamForPath(resourcePath);
        } catch (IOException e) {
    ...
    private InputStream getInputStreamForPath(String resourcePath) throws IOException {
    ...
    private String stripPrefix(String resourcePath) {
    ...
    private  InputStream loadFromFile(String path) throws IOException {
        if (logger.isDebugEnabled()) {
            logger.debug("Opening util [" + path + "]...");
        }
        return new FileInputStream(path);
    }

    private  InputStream loadFromUrl(String urlPath) throws IOException {
     ...
     * @throws AuthenticationException if the text cannot be loaded
     */
    public void load(String iniConfig) throws AuthenticationException {
     ...
     * @throws AuthenticationException if unable
     */
    public void load(InputStream is) throws AuthenticationException {
        if (is == null) {
    ...
    private void addSection(String name, StringBuilder content) {
        if (content.length() > 0) {
            ...
            String cleaned = StringUtils.trimLeadingWhitespace(contentString);
            if (cleaned != null) {
    ...
    public void load(Scanner scanner) {
        ...
        while (scanner.hasNextLine()) {
            ...
            String line = StringUtils.trimLeadingWhitespace(rawLine);
    ...
    protected static boolean isSectionHeader(String line) {
        String s = StringUtils.trimLeadingWhitespace(line);
     ...
     * {@link #getName() master} unique within an {@link Ini} instance.
     */
    public static class Section implements Map<String, String> {
        ...
        public static boolean isContinued(String line) {
        ...
        //Protected to access in a test case - NOT considered part of Shiro's public API
        public static String[] splitKeyValue(String keyValueLine) {
            String line = keyValueLine.trim();
            if (line == null) {
                return null;
            }
            ...
            String key = StringUtils.trimLeadingWhitespace(keyBuffer.toString());
            String value = StringUtils.trimLeadingWhitespace(valueBuffer.toString());
        ...
        private static Map<String, String> toMapProps(String content) {
            ...
            while (scanner.hasNextLine()) {
                line = StringUtils.trimLeadingWhitespace(scanner.nextLine());
                if(line.equals(""))
                	continue;
                if (isContinued(line)) {
