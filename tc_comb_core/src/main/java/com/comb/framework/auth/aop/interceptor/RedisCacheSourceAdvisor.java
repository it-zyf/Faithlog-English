...
//@SuppressWarnings({"unchecked"})
//@Service
public class RedisCacheSourceAdvisor extends StaticMethodMatcherPointcutAdvisor implements InitializingBean {
...
//    private Set<Class<?>> set = new HashSet<>();
    ...
    @Autowired
    private RedisCacheAnnotationMethodInterceptor redisCacheAnnotationMethodInterceptor;
...
//        }
        Method m = method;
        Annotation a = redisCacheAnnotationMethodInterceptor.getAnnotation(m, targetClass);
        if(a != null){
            return true;
        }
...
//        for( Class<? extends Annotation> annotationClass : redisCacheAnnotationMethodInterceptor.getAnnotationClass() ){
...
//        }
    ...
    public void afterPropertiesSet() throws Exception {
        ...
        Assert.notNull(redisCacheAnnotationMethodInterceptor, "casAnnotationMethodInterceptor must not be null!");
        setAdvice(redisCacheAnnotationMethodInterceptor);
        this.setOrder(3);
    }
