...
function homedata() {
    ...
    AddUser(text5,text1,textTime);
}
function retentionTable(server,start,end) {
...
//数据
function AddUser(server,start,end) {
    $.ajax({
        type: "post",
        ...
        success: function (ob) {
            // chartDaily(ob);
            retentionTable(server,start,end)
        }
    });
}
