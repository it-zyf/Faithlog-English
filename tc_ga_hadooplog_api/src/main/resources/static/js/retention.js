        ...
        columns: [
            {
                ...
                formatter: function (value, row, index) {//自定义显示，这三个参数分别是：value该行的属性，row该行记录，index该行下标
                    var index=row.logTime.indexOf(' ')
                    return row.logTime.substring(0,index)
                }
            },
            {
                title: '新增注册',
...
function tableRetain(data) {
    ...
    for (var i = 0; i < data.data[2].length; i++) {
        $("#real-tbodyT").append(
            "<tr><td>" + data.data[2][i].logTime.substring(0, data.data[2][i].logTime.indexOf(' ')) +
            "</td><td>" + (Math.floor(data.data[2][i].dayRetain * 100) / 100) +
            "</td><td>" + (Math.floor(data.data[2][i].threeRetain * 100) / 100) +
            "</td><td>"+(Math.floor(data.data[2][i].weekRetain * 100) / 100)+"</td></tr>"
        )
    }
}
