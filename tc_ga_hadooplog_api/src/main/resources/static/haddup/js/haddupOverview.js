...
function homedata(text1, textTime) {
  ...
  $.ajax({
    type: "post",
    url: allAnalysisRequestURL + "frontEnd/queryAllTrend",
    async: true,
    data: {
      serverId: text5,
      startTime: text1,
  ...
  var t = $("#real-datatableAll").bootstrapTable({
    ...
    columns: [{
      ...
      },
      {
        ...
        width: 300,
        formatter: function (value, rows, index) {
          var fufeilv = rows.payNumber / rows.activeNumber;
          fufeilv = Number(fufeilv * 100).toFixed(2);
          fufeilv += "%"
          if (fufeilv == "NaN%") {
            fufeilv = 0
          }
          return "<span class='firstin_num'>" + fufeilv + "</span>"
        }
      },
    ]
  });
...
window.setInterval(function () {
  ...
  $.ajax({
    ...
    success: function (ob) {
      ...
      $("#huo").text('+' + ob.rows.numOfpay) //付费玩家
      $("#money").text('+' + ob.rows.payAmount) //付费金额
      var fufeilv = ob.rows.numOfpay / ob.rows.numOfactive //付费率=付费玩家/活跃玩家
      $('#activation').text('+' + fufeilv) //首次激活---改为付费率
      // $("#ren").text(c + "%");
...
function onreal(ob) {
  ...
  for (var i = ob.data.length - 1; i >= 0; i--) {
    var fufeilv = ob.data[i].payNumber / ob.data[i].activeNumber
    fufeilv = Number(fufeilv * 100).toFixed(2)
    fufeilv += "%"
    if (fufeilv == "NaN%") {
      fufeilv = 0
  ...
  $("#real-datatableM").bootstrapTable({
    method: 'post',
    cache: false,
    height: 560,
    striped: true,
...
function Activation(data) {
  ...
  for (var i = data.data.length - 1; i >= 0; i--) {
    var date = data.data[i].timedate
    var rq = data.data[i].rate
    // var rq = (rq * 100) .toFixed(2)
    // var fufeilv = data.data[i].payNumber/data.data[i].activeNumber
    // fufeilv = Number(fufeilv *100).toFixed(2)
    // if(fufeilv == "NaN"){
    //     fufeilv =0
    // }
...
//付费账号
function PayAccount(data) {
  var chart4 = null;
  var arr3 = [];
  var arr7 = []
  for (var i = data.data.length - 1; i >= 0; i--) {
    var someDate = data.data[i].timedate
    var rq = data.data[i].payNumber
    arr3.push(rq)
    arr7.push(someDate)
    //	console.log(arr)
  }
  chart4 = Highcharts.chart('div-N', {
    chart: {
      // type: 'column',
      backgroundColor: {
        stops: [
