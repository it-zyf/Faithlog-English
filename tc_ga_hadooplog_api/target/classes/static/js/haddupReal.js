...
function countDown(){
    ...
    function getCountdown (){
        ...
        if( m == 0 && s == 0 ){
            ...
            $.ajax({
                type: "post",
                url: "http://192.168.0.116:8323/selectStart",//启动时间点
                async: true,
                data: {
                ...
                success: function (data) {
                    ...
                    for (var i = 0; i < data.row1.length; i++) {
                        var rq = parseInt(data.row1[i].timeCount)
                        todaydateall.push(rq)
                    }
                    startTime(todaydateall,data.row2)
            ...
            // $.ajax({
            //     type: "post",
            //     url: "http://152.136.218.252:8323/selectStartTime",//启动时间点
            //     async: true,
            //     data: {
            ...
            //             var rq = parseInt(data.data[i].timeCount)
            //             todaydateall.push(rq)
            //         }
            //         $("#page-inner").mLoading("hide");
            //     }
...
homedata()
function todaytime(datatoday,data) {
    // fetch('http://192.168.0.116:8083/selectStartTime',{
    //     method:'POST',
    //     body: new URLSearchParams({startTime:start,endTime:end}),
    // }).then((res) => res.json())
    //     .then((res)=>{
    //         startTime(datatoday,res)
    //         $("#page-inner").mLoading("hide");
    //     })
    $.ajax({
        type: "post",
...
function startTime(valueall,data) {
	...
	for (var i = 0; i < data.length; i++) {
		var date=data[i].times
		var index=date.indexOf(' ')
		var someDate = date.substring(index,index+6);
		var rq = parseInt(data[i].timeCount)
		arr.push(rq)
		arr4.push(someDate)
	}
...
function delayTime(startTime,endTime) {
    ...
    $.ajax({
        type: "post",
        url: "http://152.136.218.252:8323/selectStartRetarder",//接口-延迟时间
        async: true,
        data: {
            startTime:startTime,
            endTime:endTime
...
function delay(data) {
    ...
    for(var i=0;i<data.coldStartTime.length;i++){
        var num=data.coldStartTime[i].timeCount
        var ran=data.coldStartTime[i].timeNum
        acount+=parseInt(num)
        number.push(num)
        range.push(ran)
    }
    for(var i=0;i<data.coldStartTime.length;i++){
        var ra=(parseInt(data.coldStartTime[i].timeCount))/acount
...
function ipmap(startTime,endTime) {
    ...
    $.ajax({
        type: "post",
        url: "http://152.136.218.252:8323/selectRegion",//接口-ip地质分布
        async: true,
        data: {
            startTime:startTime,
            endTime:endTime
...
function  internetCon(startTime,endTime) {
    $.ajax({
        type: "post",
        url: "http://152.136.218.252:8323/selectNetWork",//网络连接
        async: true,
        data: {
            startTime:startTime,
            endTime:endTime
...
function internet(data) {
    ...
    for(var i=0;i<data.networkStates.length;i++){
        var nameNet=data.networkStates[i].timeNum
        if(nameNet==='0'){
            nameNet='Unkown'
        }else if(nameNet==='1'){
            nameNet='Wifi'
        ...
        var num=data.networkStates[i].timeCount
        var obj={}
        obj.value=num
        obj.name=nameNet
        datashow.push(obj)
...
function querytime(value) {
    ...
    $.ajax({
        type: "post",
        url: "http://192.168.0.116:8323/selectStartTime",//启动时间点
        async: true,
        data: {
            startTime:starttimetime,
            endTime:endtimetime
        ...
        success: function (ob) {
            ...
            startTime(todaydateall,ob.data);//启动时间点
            $("#page-inner").mLoading("hide");
        }
    });
}
function querydurationTime(valuestart,valueend) {
    ...
    $.ajax({
        type: "post",
        url: "http://192.168.0.116:8323/selectTimeQuantum",
        async: true,
        data: {
