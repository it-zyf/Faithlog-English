...
function retentionTable(startTime, endTime) {
  ...
  var t = $("#real-datatableAll").bootstrapTable({
      url: requestURL + 'analysis/rechargeRanking',
      method: 'post',
      dataType: "json",
      ...
      columns: [{
        ...
        },
        {
          ...
          formatter: function (value, row, index) {
            if (window.location.host == "107.155.50.61:9100") { //泰
              if (row.channelId == 1000) {
                return "google"
              } else if (row.channelId == 2000) {
                return "appstore"
              } else {
                return row.channelId
              }
            } else { //英
              if (row.channelId == 111092021901) {
                return "googleplay"
              } else if (row.channelId == 131092101103) {
                return "appstore"
              } else {
                return row.channelId
              }
