var serviceNull = []; //全局变量--总区服
var channelNull = []; //全局变量--总渠道号
homedata()
...
function onblu() {
  ...
  $.ajax({
    type: "post",
    url: allAnalysisRequestURL + "frontEnd/queryRetain",
    async: true,
    data: {
      serverId: serverSub,
      startTime: texts,
...
function retentionTable(ob, text5, text1, textTime) {
  ...
  var t = $("#real-datatableAll").bootstrapTable({
    // url: 'http://192.168.0.116:9110/frontEnd/queryRetain',
    method: 'post',
    dataType: "json",
    data: ob.data,
    // height:360,
...
function tableRetain(data) {
  ...
  for (var i = data.data.length - 1; i >= 0; i--) {
    var one = (data.data[i].dayRetain / data.data[i].dayAccountCount * 100).toFixed(2)
    var two = (data.data[i].threeRetain / data.data[i].threeAccountCount * 100).toFixed(2)
    var three = (data.data[i].weekRetain / data.data[i].weekAccountCount * 100).toFixed(2)
    $("#real-tbodyT").append(
      "<tr><td>" + data.data[i].timedate +
      "</td><td>" + (isNaN(one) ? 0 : one) +
      "</td><td>" + (isNaN(two) ? 0 : two) +
      "</td><td>" + (isNaN(three) ? 0 : three) + "</td></tr>"
    )
  }
}
...
function AddUser(data) {
  ...
  for (var i = data.data.length - 1; i >= 0; i--) {
    ...
    var tm = Math.floor(data.data[i].dayRetain / data.data[i].dayAccountCount * 100);
    var rq = Math.floor(data.data[i].threeRetain / data.data[i].threeAccountCount * 100);
    var we = Math.floor(data.data[i].weekRetain / data.data[i].weekAccountCount * 100);
    if (isNaN(tm) || tm == Infinity) {
      tm = 0
    }
    if (isNaN(rq) || rq == Infinity) {
      rq = 0
    }
    if (isNaN(we) || we == Infinity) {
      we = 0
    }
    arr5.push(tm)
    arr6.push(rq)
