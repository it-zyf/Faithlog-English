...
function countDown() {
    ...
    function getCountdown() {
        ...
        if (m == 0 && s == 0) {
            ...
            $.ajax({
                type: "post",
                url: allAnalysisRequestURL+"frontEnd/selectNumOfBreakdown",//启动时间点
                async: true,
                data: {
...
function showPart(start, end) {
    $.ajax({
        ...
        success: function (data) {
            ...
            }else if(data.data.number == undefined){
                numberShow.innerHTML = 0
            }else{
...
function cpuchart(data) {
    ...
    if(data.data.breakdown_cpu == undefined ){
        return false;
    }
...
function gpucharts(data) {
    ...
    var numte = 0
    var accountall = 0
    if(data.data.breakdown_gpu == undefined ){
        return false;
    }
