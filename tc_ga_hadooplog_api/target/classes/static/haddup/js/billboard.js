$(document).ready(function(){
	var table_width =$(window).width()-300 + 'px'
	$(".replace_area").css("width",table_width);
	if($(window).width() < 1920){
		$(".replace_area").css("width","0");
	}
})
...
//	$("body").mLoading("show")
	...
	var channelId = $("#reg_channel").val();//渠道号
	if(district_service!=null){
		district_service = district_service.join(',')
	};
//	if(rank_type!=null){
//		rank_type = rank_type.join(',')
//	};
	if(rank_type == 80 ||rank_type == 81||rank_type == 82||rank_type == 83){
		$("#datatable").bootstrapTable('destroy');
		var t = $("#datatable").bootstrapTable({
			url: '../game_force_list.action',
			method: 'post',
			dataType: "json",
			cache: false,
...
//				},
	...
	}else if(rank_type == 8||rank_type==36||rank_type==37||rank_type==38||rank_type==64||rank_type==39||rank_type==40||rank_type==41||rank_type==42||rank_type==43||rank_type==44||rank_type==45||rank_type==67||rank_type==67||rank_type==68||rank_type==69||rank_type==70||rank_type==71||rank_type==72||rank_type==73||rank_type==74||rank_type==75||rank_type==76||rank_type==77||rank_type==78){
		$("#datatable").bootstrapTable('destroy');
		var t = $("#datatable").bootstrapTable({
			url: '../game_force_list.action',
			method: 'post',
	...
	}else if(rank_type==84||rank_type==85||rank_type==86||rank_type==87||rank_type==88){
		$("#datatable").bootstrapTable('destroy');
		var t = $("#datatable").bootstrapTable({
			url: '../game_force_list.action',
			method: 'post',
...
//				},
		...
		t.on('load-success.bs.table', function(data) {
			...
			console.log($("#replace_table").html());
