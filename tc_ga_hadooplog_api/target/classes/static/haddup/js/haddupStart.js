...
function countDown(){
    ...
    function getCountdown (){
        ...
        if( m == 0 && s == 0 ){
            ...
            $.ajax({
                type: "post",
                url: allAnalysisRequestURL+"frontEnd/selectStart",//启动时间点
                async: true,
                data: {
...
function todaytime(datatoday,start,end) {
    $.ajax({
        type: "post",
        url: allAnalysisRequestURL+"frontEnd/selectStartTime",//启动时间点
        async: true,
        data: {
            startTime:start,
            endTime:end,
...
function startTime(valueall,data,timePointToday) {
    ...
    if(data.length===0){
        for (var i = 0; i < timePointToday.length; i++){
            ...
            var date=timePointToday[i]
            var index=date.indexOf(' ')
            var index = date
            var someDate = date.substring(11,16);
            arr4.push(someDate)
        }
    }else{
...
function delayTime(startTime,endTime) {
    $.ajax({
        type: "post",
        url: allAnalysisRequestURL+"frontEnd/selectStartRetarder",//接口-延迟时间
        async: true,
        data: {
            startTime:startTime,
            endTime:endTime
...
function delay(data) {
    ...
    var rate=[]
    var acount=0
    if(data.coldStartTime == undefined){
        return false;
    }else{
...
function ipmap(startTime,endTime) {
    $.ajax({
        type: "post",
        url: allAnalysisRequestURL+"frontEnd/selectRegion",//接口-ip地质分布
        async: true,
        data: {
            startTime:startTime,
            endTime:endTime
...
function  internetCon(startTime,endTime) {
    $.ajax({
        type: "post",
        url: allAnalysisRequestURL+"frontEnd/selectNetWork",//网络连接
        async: true,
        data: {
            startTime:startTime,
            endTime:endTime
...
function querydurationTime(valuestart,valueend) {
    ...
    $.ajax({
        type: "post",
        url: allAnalysisRequestURL+"frontEnd/selectTimeQuantum",
        async: true,
        data: {
